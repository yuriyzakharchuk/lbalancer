...
class server {
   public:
    ...
    explicit server(helpers::configurator &configurator)
        ...
        frontend_ssl_context_.set_options(
        ...
        session::ssl_session::load_server_certificate(
            frontend_ssl_context_, configurator.default_ssl_certificate(),
            configurator.default_ssl_private_key());

        for (const auto &binding : configurator.construct()) {
            std::make_shared<frontend::frontend>(
                binding.first, service_pool_.service(), frontend_ssl_context_,
                backend_ssl_context_, backend::backend_pool(binding.second))
                ->start_accept();
        }
    }

    void run() { service_pool_.run_all(); }

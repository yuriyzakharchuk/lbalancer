...
http_session::http_session(socket_t &&frontend_socket, backend_t &backend)
    : backend_(backend),
...
void http_session::start_session() {
    ...
    auto endpoints{resolver.resolve(backend_.server(), backend_.port())};
    backend_stream_.async_connect(
        endpoints,
        [chh = shared_from_this()](
            beast::error_code ec,
            const http_session::resolver_t::results_type::endpoint_type &) {
            if (ec) {
...
void http_session::frontend_read() {
    ...
    http::async_read(frontend_stream_, frontend_buffer_, frontend_request_,
                     beast::bind_front_handler(&http_session::frontend_on_read,
                                               shared_from_this()));
}

void http_session::frontend_write() {
    http::async_write(
        frontend_stream_, frontend_response_,
        beast::bind_front_handler(&http_session::frontend_on_write,
                                  shared_from_this()));
}
...
void http_session::handle_frontend_request(
...
void http_session::backend_read() {
    ...
    http::async_read(backend_stream_, backend_buffer_, backend_response_,
                     beast::bind_front_handler(&http_session::backend_on_read,
                                               shared_from_this()));
}

void http_session::backend_write() {
    http::async_write(backend_stream_, backend_request_,
                      beast::bind_front_handler(&http_session::backend_on_write,
                                                shared_from_this()));
}
...
void http_session::handle_backend_response(

...
namespace lb::session {
    ...
    public:
        ...
        static inline void
        load_server_certificate(ssl_context_t& context,
        ...
        static inline void
        verify_certificate(ssl_context_t& context) {
    ...
    private:
        ...
        void
        frontend_on_read(beast::error_code error, std::size_t bytes_transferred);

        void
        frontend_on_write(beast::error_code error, std::size_t bytes_transferred);

        void
        handle_frontend_request(http::request<http::string_body>&& request);
        ...
        void
        backend_on_read(beast::error_code error, std::size_t bytes_transferred);

        void
        backend_on_write(beast::error_code error, std::size_t bytes_transferred);

        void
        handle_backend_response(http::response<http::string_body>&& response);

        void
        backend_on_resolve(beast::error_code,
                           const resolver_t::results_type&);

        void
        backend_on_connect(beast::error_code ec);

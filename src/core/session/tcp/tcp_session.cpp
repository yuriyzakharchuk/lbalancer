...
tcp_session::tcp_session(socket_t&& frontend_socket, backend_t& backend,
...
void tcp_session::start_session() {
    frontend_socket_.async_receive(
        fb_buffer_.prepare(buffer_len_), 0,
        [c = shared_from_this()](const boost::system::error_code& error,
            ...
            if (!error) {
                if (bytes_transferred > 0) {
                    ...
                    auto endpoints{resolver.resolve(c->backend_.server(),
                                                    c->backend_.port())};
                    boost::asio::async_connect(
                        c->backend_socket_, endpoints,
...
void tcp_session::backend_read() {
    backend_socket_.async_receive(
        bf_buffer_.prepare(buffer_len_), 0,
        [c = shared_from_this()](const boost::system::error_code& error,
                                 std::size_t bytes_transferred) {
            if (!error) {
                if (bytes_transferred > 0) {

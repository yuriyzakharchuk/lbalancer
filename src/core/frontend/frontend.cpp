...
frontend::frontend(const meta_frontend &meta_frontend,
    ...
    tcp::endpoint endpoint{
        *resolver.resolve(std::string_view(meta_frontend.address),
...
void frontend::start_accept() {
    acceptor_.async_accept(boost::asio::make_strand(service_),
                           beast::bind_front_handler(&frontend::handle_accept,
                                                     shared_from_this()));
}
...
void frontend::handle_accept(const boost::system::error_code &error,
    ...
    if (!error) {
        switch (mode_) {
            case mode::tcp: {
                std::make_shared<tcp_session>(std::move(frontend_socket),
                                              backend_pool_.next_backend(),
                                              8192)
                    ->start_session();  // TODO: read this from config
                break;
            }
            case mode::http: {
                std::make_shared<http_session>(std::move(frontend_socket),
                                               backend_pool_.next_backend())
                    ->start_session();
                break;
            }
            case mode::ssl: {
                ...
                std::make_shared<ssl_session>(
                    boost::asio::make_strand(service_), frontend_context_,
                    backend_context_, std::move(frontend_socket),
                    backend_pool_.next_backend())
                    ->start_session();
                break;
            }
